...
export class TimelineEngine extends Emitter<EventTypes> implements ITimelineEngine {
   ...
   */
  setTime(time: number, isTick?: boolean): boolean {
    const result = isTick || this.emit('beforeSetTime', { time, engine: this });
    if (!result) return false;
    ...
    if (isTick) this.emit('setTimeByTick', { time, engine: this });
    else this.emit('afterSetTime', { time, engine: this });
    return true;
  }
