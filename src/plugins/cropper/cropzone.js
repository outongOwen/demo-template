...
import { clamp, addListener } from './util';
...
class Cropzone extends Rect {
  ...
  _addEventHandler() {
    ...
    addListener(document, 'keydown', this._onKeyDown.bind(this));
    addListener(document, 'keyup', this._onKeyUp.bind(this));
  }
  ...
  _fillInnerRect(ctx) {
    ...
    ctx.fillRect(x[0], y[0], textWidth + 10, 22);
    ctx.fillStyle = 'rgb(255, 255, 255)';
    ctx.fillText(txt, x[0] + 5, y[0] + 2);
    ctx.restore();
    const { left, top, width, height } = this;
    this.fire('CropzonePosition', {
      left,
      top,
      width,
      height
  ...
  _onScaling(fEvent) {
    ...
    this._scale(1);
    this.set(this._scaleLimit(settings));

    this.canvasEventTrigger[events.OBJECT_SCALED](this);
  }

  _scaleLimit(settings) {
    ...
    if (this.presetRatio) {
      if (this.presetRatio > 1) {
        ...
        limitWidth = limitHeight * this.presetRatio;
      } else {
        limitWidth = min * this.options.limitPercent;
        limitHeight = limitWidth / this.presetRatio;
      }
    } else {
      limitWidth = min * this.options.limitPercent;
      limitHeight = min * this.options.limitPercent;
    }
    if (settings.width < limitWidth) {
      settings.width = limitWidth;
    }
    if (settings.height < limitHeight) {
      settings.height = limitHeight;
    }
    return settings;
  }
  ...
  _getCropzoneRectInfo() {
    ...
    const { top: rectTop, left: rectLeft, width: rectWidth, height: rectHeight } = this.getBoundingRect(false, true);
  ...
  _resizeCropZone({ x, y }, corner) {
    const { rectWidth, rectHeight, rectTop, rectLeft, rectBottom, rectRight, canvasWidth, canvasHeight } =
...
classRegistry.setClass(Cropzone);
classRegistry.setSVGClass(Cropzone);
