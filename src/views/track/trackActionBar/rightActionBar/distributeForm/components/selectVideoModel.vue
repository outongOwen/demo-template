<template>
  ...
  >
    ...
    <div>
      <div class="flex">
        <n-input v-model:value="searchObj.videoId" autosize class="m-b-15px min-w-300px"></n-input>
        <div>
          <n-button type="primary" class="ml-15px" @click="getList">搜索</n-button>
          <n-button class="ml-15px" @click="resetSearchForm">重置</n-button>
        </div>
      </div>
      <n-data-table
    ...
    <template #action>
      <n-button size="small" style="margin-top: 10px; margin-right: 20px" @click="closeModal">取消</n-button>
      <n-button size="small" type="primary" style="margin-top: 10px; margin-right: 20px" @click="sure">确认</n-button>
    </template>
  </n-modal>
</template>
...
const getList = async () => {
  ...
  params.videoId = searchObj.value.videoId ? searchObj.value.videoId.split(',').map(v => Number(v)) : [];
  ...
  try {
    ...
    data.content.forEach((v: any) => {
      if (!getFirstHis.find((vv: any) => vv === v.firstClassCode)) {
        ...
        axiosList.push(getDistributeEnum(queryParams));
      }
    });
    Promise.allSettled(axiosList).then(values => {
      values.forEach((v: any) => {
        if (v.value.result && v.value.result.length) {
          secondClassCodeData = secondClassCodeData.concat(v.value.result);
        }
      });
