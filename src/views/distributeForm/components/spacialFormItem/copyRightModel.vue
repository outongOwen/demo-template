    ...
    <div class="h-40vh">
      <n-form ref="searchForm" :model="form" label-placement="left" label-width="80">
        <n-grid x-gap="12" :cols="3">
          <n-form-item-gi label="CPID" v-if="props.isCmam">
            <n-input v-model:value="form.cpId" disabled></n-input>
          </n-form-item-gi>
          <n-form-item-gi label="授权方式" v-if="props.isCmam" :rule="{required: props.isCmam,message: '请选择授权方式',trigger: ['change']}">
            <n-select
          ...
          <n-form-item-gi label="版权名称" v-if="props.isCmam">
            <n-input
              v-model:value="form.copyrightName"
              placeholder="请输入版权名称"
            ></n-input>
          </n-form-item-gi>
          <n-form-item-gi label="版权ID" :rule="{required: props.isCmam,message: '请输入正确的版权ID',trigger: ['change']}">
            <n-input
          ...
          <n-gi>
            ...
            <n-button @click="getCopyRightList" type="primary">搜索</n-button>
          </n-gi>
        </n-grid>
      </n-form>
      <n-data-table
...
const saveCopyId = () => {
  ...
  const selected = queryCopyRightList.value.find(
    (v: any) => v.copyrightId === multipleSelection
  )
