<template>
  <div class="flex w-100%">
    <n-select clearable class="w-150px m-r-10px" v-model:value="formData.optType" :options="optTypeOptions"></n-select>
    <n-select
    ...
    <div v-show="formData.optType" class="flex">
      <n-input class="w-50% m-r-10px" v-model:value="formData.pid"></n-input>
  ...
  getDistributeEnum(
    {
      parmStr: parmStr,
      urlCode: 'pid_list',
    },false).then((data: Record<string, any>) => {
