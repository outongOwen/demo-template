        ...
        <template v-for="item in queryArrayLeft" :key="item.key">
          <n-form-item v-show="item.show" :path="item.key" :rule="item.show?item.rules:{}" :label="item.label">
            <template v-if="item.type === 'input'">
              <n-input v-model:value="formData[item.key]" :show-count="!!item.length" :maxlength="item.length" :placeholder="item.holder"></n-input>
            </template>
            ...
            <template v-if="item.type === 'select'">
              <n-select clearable v-model:value="formData[item.key]" :options="enumOptionsList[item.optName]" :placeholder="item.holder"></n-select>
            </template>
            <template v-if="item.type === 'component'">
              <component :is="item.component" :enumOptionsList="enumOptionsList"></component>
            </template>
          </n-form-item>
        </template>
      </n-gi>
          ...
          <n-form-item v-show="item.show" :path="item.key" :rule="item.show?item.rules:{}" :label="item.label">
            ...
            <template v-if="item.type === 'datetime'">
              <n-date-picker
                class="w-100%"
                type="datetime"
                :valueF-format="item.valueFormat"
                v-model:value="formData[item.key]"
...
import {ListCode, queryArrayLeft, queryArrayRight} from "@/views/distributeForm/hooks/formInitMap";
import {getQueryString, jsonToQuery} from "@/utils";
import {getAllEnum} from "@/views/distributeForm/hooks/getAllList";
...
watch(() => formData.platformListValue,async (newVal: number[]) => {
    isInitialAssetIdFn()
    if (isMixedEdit) {
...
watch(()=>isMedia.value,(val:boolean)=>{
...
const banit3v3 = () => {
  nextTick(() => {
    const right = is3v3
    if (!right) {
      const content = formInitMap.find(
        (d: any) => d.key === 'contentForm'
      )
      content.show = right
    }
  })
